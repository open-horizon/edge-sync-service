		...
		case i, ok := <-q.objectDataVerifyQueue:
			if ok {
				...
				if common.NeedDataVerification(meta) {
					...
					// if err := store.UpdateObjectDataVerifiedStatus(meta.DestOrgID, meta.ObjectType, meta.ObjectID, true); err != nil {
					// 	if log.IsLogging(logger.ERROR) {
					// 		log.Error("Failed to set metaData.DataVerified to true for object %s/%s/%s", meta.DestOrgID, meta.ObjectType, meta.ObjectID)
					// 	}
					// 	common.ObjectLocks.Unlock(lockIndex)
					// 	return
					// }
