...
func (store *MongoStorage) addUsersToACLHelper(collection string, aclType string, orgID string, key string, users []common.ACLentry) common.SyncServiceError {
	...
	for i := 0; i < maxUpdateTries; i++ {
		...
		fmt.Printf("integratedUsernames at beginning is: %d\n", len(integratedUsernames))

		// If the entry in database is not in the input list, add it to input list
		for _, entry := range result.Users {
			add := true
			entryUserTypeInDB := entry.ACLType
			...
			for _, user := range users {
				...
				userTypeFromRequest := user.ACLType
				userNameFromRequest := user.Username
				fmt.Printf("usernameInDB: %s:%s:%s, usernameFromRequest: %s:%s:%s\n", entryUserTypeInDB, entryUsernameInDB, entry.ACLRole, userTypeFromRequest, userNameFromRequest, user.ACLRole)
				if entryUserTypeInDB == userTypeFromRequest && entryUsernameInDB == userNameFromRequest {
					fmt.Println("usernameInDB == usernameFromRequest, add set to false")
					add = false
					break
				}
			}
			...
			if add {
				fmt.Printf("add == true, add entry %s to usernames \n", entry)
				integratedUsernames = append(integratedUsernames, entry)
				fmt.Printf("integratedUsernames len now is: %d \n", len(integratedUsernames))
			}
		}

		fmt.Printf("integratedUsernames len before exeuting db update is: %d\n", len(integratedUsernames))
	...
	for i := 0; i < maxUpdateTries; i++ {
		...
		for _, user := range users {
			...
			for i, entry := range result.Users {
				entryUserTypeInDB := entry.ACLType
				entryUsernameInDB := entry.Username
				fmt.Printf("input username: %s:%s, username in db: %s:%s\n", userTypeFromRequest, userNameFromRequest, entryUserTypeInDB, entryUsernameInDB)
				if entryUserTypeInDB == userTypeFromRequest && entryUsernameInDB == userNameFromRequest {
					fmt.Println("Find user in db.")
					...
					fmt.Printf("deleting %s:%s:%s from list \n", result.Users[i].ACLType, result.Users[i].Username, result.Users[i].ACLRole)
					result.Users[i] = result.Users[len(result.Users)-1]
					result.Users = result.Users[:len(result.Users)-1]
					deleted = true
					fmt.Printf("usernames len is: %d now\n", len(result.Users))
					break
				}
